...
public class ShearObjective extends Objective implements Listener {
    ...
    public void onShear(PlayerShearEntityEvent event) {
        ...
        if (color != null && !((Sheep) event.getEntity()).getColor().toString().equalsIgnoreCase(color))
            return;
        SheepData data = (SheepData) dataMap.get(uuid);
        ...
        if (checkConditions(uuid)) {
            ...
            if (data.getAmount() <= 0)
                ...
                MessageUtils.sendNotify(uuid, "sheep_to_shear", new String[]{String.valueOf(data.getAmount())},
                        "sheep_to_shear,info");
        }
    }
    ...
    public String getProperty(String name, UUID uuid) {
        if (name.equalsIgnoreCase("left")) {
            return Integer.toString(((SheepData) dataMap.get(uuid)).getAmount());
        } else if (name.equalsIgnoreCase("amount")) {
            return Integer.toString(amount - ((SheepData) dataMap.get(uuid)).getAmount());
        }
        return "";
    }
    ...
    public static class SheepData extends ObjectiveData {
        ...
        public SheepData(String instruction, UUID uuid, String objID) {

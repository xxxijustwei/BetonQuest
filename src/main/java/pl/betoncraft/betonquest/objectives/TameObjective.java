...
public class TameObjective extends Objective implements Listener {
    ...
    public void onTaming(EntityTameEvent event) {
        if (event.getOwner() instanceof Player) {
            ...
            TameData playerData = (TameData) dataMap.get(uuid);
            switch (type) {
                case WOLF:
                    if (entity.getType().equals(EntityType.WOLF) && checkConditions(uuid)) {
                        playerData.subtract();
                    }
                    break;
                case OCELOT:
                    if (entity.getType().equals(EntityType.OCELOT) && checkConditions(uuid)) {
                        playerData.subtract();
                    }
                    break;
                case HORSE:
                    if (entity.getType().equals(EntityType.HORSE) && checkConditions(uuid)) {
                        playerData.subtract();
                    }
                    break;
                default:
    ...
    public String getProperty(String name, UUID uuid) {
        if (name.equalsIgnoreCase("left")) {
            return Integer.toString(amount - ((TameData) dataMap.get(uuid)).getAmount());
        } else if (name.equalsIgnoreCase("amount")) {
            return Integer.toString(((TameData) dataMap.get(uuid)).getAmount());
        }
        return "";
    }
    ...
    public static class TameData extends ObjectiveData {
        ...
        public TameData(String instruction, UUID uuid, String objID) {

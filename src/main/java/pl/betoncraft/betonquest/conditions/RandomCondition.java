...
public class RandomCondition extends Condition {
    ...
    public RandomCondition(Instruction instruction) throws InstructionParseException {
        ...
        try {
            valueMax = new VariableNumber(values[0]);
            rangeOfRandom = new VariableNumber(values[1]);
    ...
    public boolean check(UUID uuid) throws QuestRuntimeException {
        ...
        int temp = generator.nextInt(rangeOfRandom.getInt(uuid)) + 1;
        return temp <= valueMax.getInt(uuid);
    }

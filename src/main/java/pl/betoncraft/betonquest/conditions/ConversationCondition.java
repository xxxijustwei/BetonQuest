...
public class ConversationCondition extends Condition {
    ...
    public boolean check(UUID uuid) throws QuestRuntimeException {
        ...
        ConversationData conversation = BetonQuest.getQuestManager().getConversation(conversationID);

        if (conversation == null) {
            throw new QuestRuntimeException("Conversation does not exist: " + conversationID);
        }

        return conversation.isReady(uuid);
    }

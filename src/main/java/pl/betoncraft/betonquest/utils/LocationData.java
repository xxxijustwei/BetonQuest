...
public class LocationData {
     ...
     */
    public LocationData(String string) throws InstructionParseException {
        // parse the vector
        String base = null;
        if (string.contains("->")) {
        ...
        if (base.startsWith("%") && base.endsWith("%")) {
            type = Type.VARIABLE;
            variable = QuestManager.createVariable(base);
        } else {
            type = Type.LOCATION;
    ...
    private Location getBaseLoc(UUID uuid) throws QuestRuntimeException {
        switch (type) {
            case LOCATION:
                return loc;
            case VARIABLE:
                ...
                String value = variable.getValue(uuid);
                try {
    ...
    /**
     * @param uuid ID of the player, needed for location resolution
     ...
     *                               be accessed
     */
    public Location getLocation(UUID uuid) throws QuestRuntimeException {
        return getBaseLoc(uuid).clone().add(vector);
    }

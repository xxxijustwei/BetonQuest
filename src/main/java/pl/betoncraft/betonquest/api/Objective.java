...
public abstract class Objective {
     ...
     */
    protected HashMap<UUID, ObjectiveData> dataMap = new HashMap<>();
    /**
     ...
     */
    public final void completeObjective(final UUID uuid) {
        // remove the objective from player's list
        removePlayer(uuid);
        BetonQuest.getInstance().getPlayerData(uuid).removeRawObjective((ObjectiveID) instruction.getID());
        if (persistent) {
            BetonQuest.getInstance().getPlayerData(uuid).addNewRawObjective((ObjectiveID) instruction.getID());
            addPlayer(uuid, getDefaultDataInstruction());
        }
        LogUtils.getLogger().log(Level.FINE,
                "Objective \"" + instruction.getID().getFullID() + "\" has been completed for player "
                        + PlayerConverter.getName(uuid)
                        + ", firing events.");
        // fire all events
     ...
     */
    public final boolean checkConditions(final UUID uuid) {
        LogUtils.getLogger().log(Level.FINE, "Condition check in \"" + instruction.getID().getFullID()
                + "\" objective for player " + PlayerConverter.getName(uuid));
     ...
     *            ID of the player
     */
    public final void newPlayer(UUID uuid) {
        String def = getDefaultDataInstruction();
        addPlayer(uuid, def);
        BetonQuest.getInstance().getPlayerData(uuid).addObjToDB(instruction.getID().getFullID(), def);
    }
        ...
        try {
            ...
            if (e.getCause() instanceof InstructionParseException) {
                LogUtils.getLogger().log(Level.WARNING, "Error while loading " + this.instruction.getID().getFullID() + " objective data for player "
                        + PlayerConverter.getName(uuid) + ": " + e.getCause().getMessage());
     ...
     *            ID of the player
     */
    public final synchronized void removePlayer(UUID uuid) {
        dataMap.remove(uuid);
        if (dataMap.isEmpty()) {
            stop();
        }
     ...
     */
    public final String getData(UUID uuid) {
        ObjectiveData data = dataMap.get(uuid);
        if (data == null) {
            return null;
        }
    ...
    public void setLabel(ObjectiveID rename) {
        instruction = new Instruction(rename, instruction.toString());
    }
    ...
    public void close() {
        ...
        for (UUID uuid : dataMap.keySet()) {
            BetonQuest.getInstance().getPlayerData(uuid).addRawObjective(instruction.getID().getFullID(),
                    dataMap.get(uuid).toString());
        }
    }
    ...
    protected static class ObjectiveData {
         ...
         */
        public ObjectiveData(String instruction, UUID uuid, String objID) {
            this.instruction = instruction;
            this.uuid = uuid;
            this.objID = objID;
        }
        ...
        protected void update() {
            QuestDataUpdateEvent event = new QuestDataUpdateEvent(uuid, objID, toString());
            Bukkit.getPluginManager().callEvent(event);
            // update the journal so all possible variables display correct
            // information
            BetonQuest.getInstance().getPlayerData(uuid).getJournal().update();
        }

...
public class NPCMoveEvent extends QuestEvent implements Listener {
    ...
    public void run(UUID uuid) throws QuestRuntimeException {
        ...
        if (PlayerConverter.getPlayer(uuid) == null) {
            currentPlayer = null;
            return;
        }
        ...
        if (CitizensWalkingListener.getInstance().isMovementPaused(npc)) {
            CitizensWalkingListener.getInstance().setNewTargetLocation(npc, firstLocation.getLocation(uuid));
        } else {
            npc.getNavigator().setTarget(firstLocation.getLocation(uuid));
        }
        currentPlayer = uuid;
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (EventID event : doneEvents) {
                    QuestManager.event(currentPlayer, event);
                }
                currentPlayer = null;
            }

...
public class CitizensHologram extends BukkitRunnable implements Listener {
    ...
    public CitizensHologram() {
        ...
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(BetonQuest.getInstance(), () -> {
            ...
            ConfigurationSection section = pack.getCustom().getYaml().getConfigurationSection("npc_holograms");
            ...
            if (interval <= 0) {
                LogUtils.getLogger().log(Level.WARNING, "Could not load npc holograms, Check interval must be bigger than 0.");
                return;
            }
            ...
            for (String key : section.getKeys(false)) {
                ...
                try {
                    ...
                    LogUtils.getLogger().log(Level.WARNING, "Invalid vector: " + settings.getString("vector"));
                ...
                if (rawConditions != null) {
                    for (String part : rawConditions.split(",")) {
                        try {
                            hologramConfig.conditions.add(new ConditionID(part));
                        } catch (ObjectNotFoundException e) {
                            LogUtils.getLogger().log(Level.WARNING, "Error while loading " + part + " condition for hologram " + key + ": " + e.getMessage());
                            LogUtils.logThrowable(e);
                        }
                    }
                }

...
public class VaultIntegrator implements Integrator {
    ...
    public void hook() {
        ...
        if (economy != null) {
            BetonQuest.getQuestManager().registerEvents("money", MoneyEvent.class);
            BetonQuest.getQuestManager().registerConditions("money", MoneyCondition.class);
            BetonQuest.getQuestManager().registerVariable("money", MoneyVariable.class);
        } else {
            LogUtils.getLogger().log(Level.WARNING, "There is no economy plugin on the server!");
        }
        if (permission != null) {
            BetonQuest.getQuestManager().registerEvents("permission", PermissionEvent.class);
        } else {

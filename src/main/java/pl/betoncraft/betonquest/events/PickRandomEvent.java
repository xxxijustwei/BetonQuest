...
public class PickRandomEvent extends QuestEvent {
    ...
    public PickRandomEvent(Instruction instruction) throws InstructionParseException {
        ...
        this.events = instruction.getList(string -> {
            ...
            VariableNumber chance = new VariableNumber(parts[0]);
            EventID id;
            try {
                id = new EventID(parts[1]);
    ...
    public void run(UUID uuid) throws QuestRuntimeException {
        ...
        for (RandomEvent event : events) {
            total += event.getChance().getDouble(uuid);
        }
        //pick as many events as given with pick optional (or 1 if amount wasn't specified)
        int pick = (this.amount != null) ? this.amount.getInt(uuid) : 1;
        while (pick-- > 0 && !events.isEmpty()) {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                current += event.getChance().getDouble(uuid);
                if (current >= found) {
                    //run the event
                    QuestManager.event(uuid, event.getId());
